...
export class Game {
  ...
  /**
   * Finds the first index in the sorted objects array that could potentially
   * be visible or colliding at the given x coordinate.
   */
  ...
  private checkCollisions() {
    ...
    // Optimization: Start from binary-searched index and use manual loop for performance
    ...
    for (let i = startIdx; i < this.level.objects.length; i++) {
      ...
      if (obj.x > pRight + 10) break; // Spatial pruning: objects are sorted by X
      if (obj.x + obj.width < pLeft - 10) continue;
  ...
  private draw() {
    ...
    // Optimization: Batch drawing calls with Path2D
    const blockPath = new Path2D();
    const spikePath = new Path2D();
    let hasBlocks = false;
    let hasSpikes = false;

    // Spatial pruning for rendering
    const startIdx = this.findFirstIndexAfter(cameraX - this.maxObjWidth);
    for (let i = startIdx; i < level.objects.length; i++) {
      const obj = level.objects[i];
      if (obj.x > cameraX + canvas.width) break; // Objects are sorted
