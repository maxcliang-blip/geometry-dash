...
export class Game {
  ...
  private checkCollisions() {
    ...
    // Optimization: Spatial pruning - only check objects near the player
  ...
  /**
   * Optimization: Use binary search to find the first object that could potentially be visible or
   * collide with the player.
   * @param minX The minimum x-coordinate of interest
   */
  private findStartIndex(minX: number): number {
    ...
    // Since objects are sorted by x, we can search for obj.x >= minX - maxObjWidth
  ...
  /**
   * Optimization: Spatial pruning. Only iterate over objects within the specified x-range.
   */
  private forEachInRange(minX: number, maxX: number, callback: (obj: GameObject) => void) {
    ...
    for (let i = startIndex; i < objects.length; i++) {
      const obj = objects[i];
      if (obj.x > maxX) break; // Optimization: Objects are sorted, so we can stop early
      callback(obj);
    }
  }
  ...
  private draw() {
    ...
    // Optimization: Spatial pruning and Path2D batching for rendering
