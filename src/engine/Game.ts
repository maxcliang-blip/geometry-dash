...
export class Game {
  ...
  private checkCollisions() {
    ...
    this.forEachInRange(pLeft - 30, pRight + 30, (obj) => {
      ...
      const currentPTop = this.player.y - this.player.height;
      const objTop = -obj.y - obj.height;
      const objLeft = obj.x;

      if (this.rectIntersect(pLeft, currentPTop, this.player.width, this.player.height, objLeft, objTop, obj.width, obj.height)) {
        if (obj.type === 'spike') {
