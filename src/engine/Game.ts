...
export class Game {
  ...
  /**
   * Performs a binary search to find the first object index that could be visible
   * or relevant for collisions, starting from targetX - maxObjWidth.
   */
  private findFirstIndexAfter(targetX: number): number {
    ...
    const minX = targetX - this.maxObjWidth;
  ...
  private checkCollisions() {
    ...
    for (let i = startIndex; i < this.level.objects.length; i++) {
      ...
      if (obj.x > pRight + 30) break; // Optimization: Objects are sorted by x
  ...
  private draw() {
    ...
    for (let i = startIndex; i < level.objects.length; i++) {
      ...
      if (obj.x > cameraX + canvas.width) break; // Optimization: Objects are sorted by x
