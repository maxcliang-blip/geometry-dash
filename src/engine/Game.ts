...
export class Game {
  ...
  private checkCollisions() {
    ...
    // Spatial pruning: Use binary search to find starting object
    ...
    for (let i = startIndex; i < this.level.objects.length; i++) {
      ...
      // Early break: Objects are sorted by x, so we can stop once we're past the player
  ...
  private draw() {
    ...
    // Populate paths for batch rendering
