...
export class Game {
  ...
  private checkCollisions() {
    ...
    // Optimization: Start from the first relevant object using binary search
    ...
    for (let i = startIndex; i < this.level.objects.length; i++) {
      ...
      // Early exit if object is too far right
      if (obj.x > pRight) break;
  ...
  private draw() {
    ...
    // Draw objects using Path2D batching
    for (let i = startIndex; i < level.objects.length; i++) {
      const obj = level.objects[i];
      // Early exit if object is beyond viewport
  ...
  /**
   * Performs a binary search to find the index of the first object whose x-coordinate
   * is greater than or equal to the targetX.
   */
  private findFirstIndexAfter(targetX: number): number {
    let low = 0;
    ...
    while (low <= high) {
      ...
      if (this.level.objects[mid].x >= targetX) {
        result = mid;
        high = mid - 1;
      } else {
        low = mid + 1;
