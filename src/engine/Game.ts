...
export class Game {
  ...
  /**
   * Efficiently iterates over level objects in the given x-range using binary search.
   * Time complexity: O(log N + M) where N is total objects and M is objects in range.
   */
  private forEachInRange(minX: number, maxX: number, callback: (obj: GameObject) => void) {
    ...
    // We search for minX - maxObjWidth because an object starting before minX might still overlap
    ...
    for (let i = startIndex; i < objects.length; i++) {
      const obj = objects[i];
      if (obj.x > maxX) break; // Past the range
      if (obj.x + obj.width < minX) continue; // Ends before range
      callback(obj);
    }
  }
  ...
  private draw() {
    ...
    // Draw objects using spatial pruning and batching
