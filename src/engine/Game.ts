...
export class Game {
  ...
  constructor(canvas: HTMLCanvasElement, level: LevelData) {
    this.canvas = canvas;
    // Optimization: Disable alpha channel since background is opaque
  ...
  private checkCollisions() {
    ...
    for (let i = startIndex; i < this.level.objects.length; i++) {
      ...
      if (obj.x > pRight + 30) break; // Early exit: objects are sorted by x
  ...
  private draw() {
    ...
    // Optimization: Batch drawing using Path2D
