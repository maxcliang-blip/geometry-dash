...
export class Game {
  ...
  private checkCollisions() {
    ...
    // Optimization: Use binary search to find objects near the player
    // Subtract maxObjWidth to include objects that start before pLeft but may overlap
    const startIndex = this.findFirstIndexAfter(pLeft - this.maxObjWidth - 10);
    const objects = this.level.objects;

    for (let i = startIndex; i < objects.length; i++) {
      const obj = objects[i];
      // Early exit: if object starts beyond player reach, no need to check further
      if (obj.x > pRight + 10) break;
  ...
  private draw() {
    ...
    // Optimization: Batch draw calls using Path2D
    const blockPath = new Path2D();
    const spikePath = new Path2D();
    let hasBlocks = false;
    let hasSpikes = false;

    // Optimization: Viewport culling using binary search
    const startIndex = this.findFirstIndexAfter(cameraX - this.maxObjWidth);
    const objects = level.objects;

    for (let i = startIndex; i < objects.length; i++) {
      const obj = objects[i];
      // Early exit: if object starts beyond viewport, stop checking
    ...
    ctx.rotate(player.rotation * DEG_TO_RAD);
