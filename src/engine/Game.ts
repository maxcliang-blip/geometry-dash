...
export class Game {
  ...
  private forEachInRange(minX: number, maxX: number, callback: (obj: GameObject) => void) {
    ...
    let startIdx = objects.length;

    // We search for the first object that COULD be in range.
    // Since objects are sorted by x, any object with x < minX - maxObjWidth
    // cannot possibly overlap with the range starting at minX.
  ...
  private draw() {
    ...
    // Viewport culling using binary search
