...
export class Game {
  ...
  private checkCollisions() {
    ...
    // Optimization: Only iterate over objects near the player
  ...
  private forEachInRange(minX: number, maxX: number, callback: (obj: GameObject) => void) {
    // Optimization: Account for max object width to include objects starting before minX but overlapping the range
  ...
  private draw() {
    ...
    // Optimization: Draw only visible objects and batch by type using Path2D
