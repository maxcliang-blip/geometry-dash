...
export class Game {
  ...
  /**
   * Performs binary search to find the first object index that could be visible/active.
   * Accounts for maxObjWidth to ensure objects starting before minX but overlapping it are found.
   */
  ...
  /**
   * Efficiently iterates over game objects within a spatial range using binary search pruning.
   */
  private forEachInRange(minX: number, maxX: number, callback: (obj: GameObject) => void) {
    ...
    for (let i = startIndex; i < this.level.objects.length; i++) {
      ...
      if (obj.x > maxX) break; // Objects are sorted by X, so we can stop here
      if (obj.x + obj.width < minX) continue; // Skip objects that end before range
      callback(obj);
    }
  }
  ...
  private checkCollisions() {
    ...
    if (!this.player.isDead && groundedOnObject) {
