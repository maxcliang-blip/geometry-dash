...
export class Game {
  ...
  /**
   * Finds the first index in the objects array that could be visible or colliding.
   * Uses binary search to find the first object where obj.x + obj.width >= minX.
   */
  private findFirstIndexAfter(minX: number): number {
    ...
    while (low <= high) {
      ...
      // We need to find the first object that might be on screen.
      // Since objects are sorted by x, we can use binary search.
      // But we must account for maxObjWidth because an object starting
      // before minX might still overlap if it's wide.
  ...
  private checkCollisions() {
    ...
    const startIndex = this.findFirstIndexAfter(pLeft);

    for (let i = startIndex; i < this.level.objects.length; i++) {
      const obj = this.level.objects[i];
      if (obj.x > pRight + 30) break; // Optimization: Objects are sorted, can break early
  ...
  private draw() {
    ...
    for (let i = startIndex; i < level.objects.length; i++) {
      ...
      if (obj.x > cameraX + canvas.width) break; // Optimization: Objects are sorted, can break early
      if (obj.type === 'block') {
