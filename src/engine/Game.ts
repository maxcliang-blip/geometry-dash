...
export class Game {
  ...
  private findStartIndex(minX: number): number {
    ...
    // We need to find the first object where obj.x + obj.width >= minX
    // Since we know maxObjWidth, we can search for obj.x >= minX - maxObjWidth
  ...
  private forEachInRange(minX: number, maxX: number, callback: (obj: GameObject) => void) {
    const startIdx = this.findStartIndex(minX);
