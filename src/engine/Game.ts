...
export class Game {
  ...
  private findFirstIndexInRange(minX: number): number {
    let low = 0;
  ...
  private forEachInRange(minX: number, maxX: number, callback: (obj: GameObject) => boolean | void) {
    // Optimization: Use binary search to find the starting index
    // We search for minX - maxObjWidth to include objects that start before minX but overlap the range
    const startIndex = this.findFirstIndexInRange(minX - this.maxObjWidth);
  ...
  private checkCollisions() {
    ...
    // Optimization: Spatial pruning using binary search range
    this.forEachInRange(pLeft - 30, pRight + 30, (obj) => {
      ...
      if (this.rectIntersect(pLeft, pTop, this.player.width, this.player.height, objLeft, objTop, obj.width, obj.height)) {
        if (obj.type === 'spike') {
          ...
          } else if (!(obj.y === 0 && this.player.y === 0)) {
            this.player.isDead = true;
            return false; // Break loop
          } else {
            groundedOnObject = true;
  ...
  private draw() {
    ...
    // Optimization: Path2D batching for geometry
    const blockPath = new Path2D();
    const spikePath = new Path2D();
    let hasBlocks = false;
    let hasSpikes = false;

    // Optimization: Spatial pruning for rendering
