...
export class Game {
  ...
  private checkCollisions() {
    ...
    // Optimization: Spatial pruning with forEachInRange reduces iterations
    // Use +/- 30px buffer to ensure we don't miss nearby objects
    this.forEachInRange(pLeft - 30, pRight + 30, (obj) => {
      // pTop must be recalculated because player.y might change during collision resolution
  ...
  /**
   * Efficiently iterates over objects within a horizontal range using binary search.
   * Optimization: O(log N + K) complexity instead of O(N).
   */
  private forEachInRange(minX: number, maxX: number, callback: (obj: GameObject) => void) {
    ...
    // Binary search for the first object that could potentially be in view
    // (accounting for object width)
    const targetX = minX - this.maxObjWidth;

    while (start <= end) {
      const mid = (start + end) >> 1;
      if (objects[mid].x >= targetX) {
        index = mid;
        end = mid - 1;
      } else {
        start = mid + 1;
    ...
    // Iterate until we pass the visible range
  ...
  private draw() {
    ...
    // Optimization: Use forEachInRange for viewport culling and Path2D for batching
    ...
    ctx.restore(); // Restore to camera transform
    ctx.restore(); // Restore to global context
