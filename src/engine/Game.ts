...
export class Game {
  ...
  private forEachInRange(minX: number, maxX: number, callback: (obj: GameObject) => boolean | void) {
    ...
    // We search for minX - maxObjWidth because an object starting before minX might still overlap it
  ...
  private draw() {
    ...
    // Draw objects
    // Optimization: Only iterate over visible objects and batch drawing with Path2D
    this.forEachInRange(cameraX - this.maxObjWidth, cameraX + canvas.width, (obj) => {
      if (obj.type === 'block') {
