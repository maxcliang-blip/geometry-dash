...
export class Game {
  ...
  private forEachInRange(minX: number, maxX: number, callback: (obj: GameObject) => void) {
    // Start search from minX - maxObjWidth to include objects that might overlap the boundary
  ...
  private draw() {
    ...
    // Draw objects using Path2D batching and spatial pruning
    ...
    ctx.restore(); // Restore player-specific transform
