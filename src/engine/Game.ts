...
export class Game {
  ...
  private checkCollisions() {
    ...
    // Optimization: Use binary search to find the first object that could possibly collide.
    // Objects are sorted by x, so we search for objects where obj.x + maxObjWidth >= pLeft - 30.
    ...
    for (let i = startIndex; i < this.level.objects.length; i++) {
      ...
      if (obj.x > pRight + 30) break; // Optimization: Objects are sorted, so we can stop early
  ...
  private draw() {
    ...
    // Optimization: Use binary search for spatial pruning during rendering
    const startIndex = this.findFirstIndexAfter(cameraX - this.maxObjWidth);

    // Draw objects - Optimization: Batch drawing calls using Path2D
