...
export class Game {
  ...
  private checkCollisions() {
    ...
    // Optimization: Use binary search to find the starting point of visible objects
    ...
    for (let i = startIndex; i < this.level.objects.length; i++) {
      ...
      if (obj.x > pRight) break; // Optimization: Stop early since objects are sorted by x
  ...
  private draw() {
    ...
    // Optimization: Use binary search for spatial pruning in rendering
    const startIndex = this.findFirstIndexAfter(cameraX - this.maxObjWidth);

    // Draw objects
    for (let i = startIndex; i < level.objects.length; i++) {
      const obj = level.objects[i];
      if (obj.x > cameraX + canvas.width) break; // Optimization: Stop early
    ...
    ctx.restore(); // Restore player transform, back to camera transform
    ctx.restore(); // Restore camera transform, back to global context
