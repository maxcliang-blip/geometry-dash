...
export class Game {
  ...
  private forEachInRange(minX: number, maxX: number, callback: (obj: GameObject) => void) {
    // Spatial pruning using binary search on sorted objects
    // We search for minX - maxObjWidth to include objects that start before minX but might overlap
  ...
  private checkCollisions() {
    ...
    if (groundedOnObject && !this.player.isDead) {
  ...
  private draw() {
    ...
    // Draw objects with spatial pruning and Path2D batching
    ...
    ctx.restore(); // Restore global context (from camera transform)
