...
export class Game {
  ...
  private checkCollisions() {
    ...
    // Optimization: Use spatial pruning to check only nearby objects
  ...
  /**
   * Optimization: Efficiently iterate over objects in a given x-range.
   * Uses binary search to find the start index.
   */
  private forEachInRange(minX: number, maxX: number, callback: (obj: GameObject) => void) {
    const objects = this.level.objects;
    const startIndex = this.findStartIndex(minX - this.maxObjWidth);
  ...
  /**
   * Binary search for the first object index where obj.x >= targetX.
   */
  private findStartIndex(targetX: number): number {
    const objects = this.level.objects;
    let low = 0;
