...
export class Game {
  ...
  private checkCollisions() {
    ...
    // Spatial pruning: Use binary search to find the first object that could possibly collide
    const startIndex = this.findFirstIndexAfter(pLeft - this.maxObjWidth - 30);

    for (let i = startIndex; i < this.level.objects.length; i++) {
      const obj = this.level.objects[i];
      if (obj.x > pRight + 30) break; // Optimization: Objects are sorted by X, so we can stop early
  ...
  private draw() {
    ...
    // Optimization: Batch drawing calls using Path2D
    const blockPath = new Path2D();
    const spikePath = new Path2D();
    let hasBlocks = false;
    let hasSpikes = false;

    // Spatial pruning: Use binary search to find the first object in the viewport
