...
export class Game {
  ...
  private draw() {
    ...
    // Viewport culling and Path2D batching
    ...
    ctx.restore(); // Restore camera transform and ground translation
  ...
  private forEachInRange(minX: number, maxX: number, callback: (obj: GameObject) => void) {
    ...
    // We search for minX - maxObjWidth to catch objects that start before minX but overlap
