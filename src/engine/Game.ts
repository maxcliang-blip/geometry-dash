...
export class Game {
  ...
  /**
   * Finds the first index in the objects array where obj.x >= minX using binary search.
   * This is used for spatial pruning in rendering and collision detection.
   */
  private findFirstIndexAfter(minX: number): number {
    ...
    while (low < high) {
      ...
      if (this.level.objects[mid].x < minX) low = mid + 1;
      else high = mid;
    }
    return low;
  }
  ...
  private checkCollisions() {
    ...
    // Optimization: Spatial pruning with binary search
    // We check objects from (pLeft - maxObjWidth) to (pRight)
    const startIndex = this.findFirstIndexAfter(pLeft - this.maxObjWidth);

    for (let i = startIndex; i < this.level.objects.length; i++) {
      const obj = this.level.objects[i];
      if (obj.x > pRight) break; // Optimization: Early exit when objects are past the player
  ...
  private draw() {
    ...
    // Optimization: Spatial pruning with binary search for rendering
    const startIndex = this.findFirstIndexAfter(cameraX - this.maxObjWidth);

    // Draw objects
    for (let i = startIndex; i < level.objects.length; i++) {
      const obj = level.objects[i];
      if (obj.x > cameraX + canvas.width) break; // Optimization: Early exit when objects are past the viewport
