...
export class Game {
  ...
  private checkCollisions() {
    ...
    // Optimization: Only check collisions for objects near the player
  ...
  /**
   * Performs binary search to find the first object index that could be visible
   * or relevant for collisions starting at minX.
   */
  private findStartIndex(minX: number): number {
    ...
    // We want the first object where obj.x + obj.width >= minX
    // Since objects are sorted by x, we search for the first index where obj.x >= minX - maxObjWidth
    const searchTarget = minX - this.maxObjWidth;

    while (low <= high) {
      const mid = Math.floor((low + high) / 2);
      if (objects[mid].x >= searchTarget) {
        result = mid;
        high = mid - 1;
      } else {
        low = mid + 1;
  ...
  /**
   * Optimized iterator that only processes objects within a specific X range.
   */
  private forEachInRange(
    ...
    for (let i = startIndex; i < objects.length; i++) {
      const obj = objects[i];
      // Since objects are sorted by x, we can stop once obj.x exceeds maxX
      if (obj.x > maxX) break;

      // Further filter: only include if the object actually overlaps the range
  ...
  private draw() {
    ...
    // Draw objects - Optimization: Use Path2D batching and spatial pruning
    ...
    ctx.restore(); // Restore player rotation transform
    ctx.restore(); // Restore camera transform (cameraX, groundY translation)
