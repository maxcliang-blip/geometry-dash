...
export class Game {
  ...
  /**
   * Finds the starting index for spatial pruning using binary search.
   * Optimization: Reduces search complexity from O(n) to O(log n).
   */
  private findStartIndex(minX: number): number {
    ...
    // Binary search for the first object that might be in range
    // We search for minX - maxObjWidth because an object could start before the viewport but overlap it
  ...
  /**
   * Iterates only over objects within the specified x-range.
   * Optimization: Significantly reduces the number of collision checks and draw calls per frame,
   * especially in large levels. Complexity: O(log n + m) where m is the number of visible objects.
   */
  private forEachInRange(minX: number, maxX: number, callback: (obj: GameObject) => void) {
    ...
    for (let i = startIndex; i < this.level.objects.length; i++) {
      ...
      if (obj.x > maxX) break;
      if (obj.x + obj.width < minX) continue;
      callback(obj);
    }
  }
