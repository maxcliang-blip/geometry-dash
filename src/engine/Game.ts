...
export class Game {
  ...
  private checkCollisions() {
    ...
    // Spatial pruning for collisions: Only check objects near the player
  ...
  /**
   * Optimized iterator that only processes objects within the given spatial range.
   * Returns false from the callback to break early.
   */
  private forEachInRange(minX: number, maxX: number, callback: (obj: GameObject) => void | boolean) {
    // Subtract maxObjWidth because an object might start before minX but still overlap the range
  ...
  private draw() {
    ...
    // Draw objects - batching using Path2D
