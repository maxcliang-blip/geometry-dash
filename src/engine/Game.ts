...
export class Game {
  ...
  private checkCollisions() {
    ...
    for (let i = startIndex; i < this.level.objects.length; i++) {
      ...
      if (obj.x > pRight + 30) break; // Optimization: Early exit if object is past collision range
  ...
  private draw() {
    ...
    // Optimization: Draw objects using spatial pruning and Path2D batching
    const startIndex = this.findFirstIndexAfter(cameraX);
    for (let i = startIndex; i < level.objects.length; i++) {
      const obj = level.objects[i];
      if (obj.x > cameraX + canvas.width) break; // Early exit if object is past viewport
  ...
  /**
   * Performs a binary search to find the first object index whose x-coordinate
   * is potentially within the viewport or collision range.
  ...
  private findFirstIndexAfter(minX: number): number {
    ...
    // Adjust search to include wide objects that might start before minX
